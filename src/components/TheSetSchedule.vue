<template>
	<div class="w-full h-[3.125rem] inline-flex flex-row items-center justify-between px-3">
		<div class="flex items-center justify-between">
            <div class="w-[1.875rem] h-[1.875rem] hover:bg-[#0f14191a] rounded-full flexCenter cursor-pointer" @click="scrollVisibil">
		...
		<div class="inline-flex justify-between items-center mr-1">
			<span  class="px-3.75 h-[1.875rem] rounded-3.75 cursor-pointer flex items-center hover:bg-[#0c14191a] mr-4 rounded-full" v-if="data.sending" 
			 @click="scrollVisibil(), sendSchedule({})">
			...
			<button class="h-[1.875rem] px-3.75 bg-[#0f1419] text-sm font-bold text-white rounded-[2rem]"
			 :class="{'opacity-70 pointer-events-none': data.error}" @click="sendSchedule(data.scheduling), scrollVisibil()">
